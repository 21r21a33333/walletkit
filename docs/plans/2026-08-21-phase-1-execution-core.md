...
> **FINALIZED & IMPLEMENTED 2026-08-22 (supersedes everything below).** Research corrected the framing twice: (1) the ABI is **stdio-wasip1, not Component Model** (Javy JS exports take no params/return, stock Go can't target components); (2) **"reuse Fystack/MoonPay" was inaccurate** — the real libraries aren't feature catalogs. **Fystack** (`github.com/fystack/programmable-policy-engine`, Go+expr-lang) is a thin generic expr evaluator; **MoonPay's Open Wallet Standard (OWS)** is itself an in-process signing gate (a *peer* of walletkit) with only 3 declarative rules (`allowed_chains`, `expires_at`, `allowed_typed_data_contracts`) + a custom `executable` (stdin `PolicyContext` JSON → stdout `{allow,reason}` — our ABI). **Decision: dropped Fystack; support MoonPay/OWS** (feature `policy-moonpay`) as `MoonPayPolicyEngine` = real OWS policy JSON, **declarative rules evaluated natively** (`allowed_chains` + `expires_at`; typed-data rule is EIP-712-only → no-op for a tx) **+ the OWS `executable` run as a user-supplied `wasip1` plugin** in the hardened `WasmPlugin` sandbox (fresh Store/run, pipes-only, fuel + `StoreLimits`, `InstancePre`) — safer than OWS's subprocess. Host mints the approval; deny short-circuits before the executable; all fail-closed. No shipped-engine wasm, no gzip. The design below is historical.
...
**Production nonce feature checklist (research 2026-08-22 vs alloy/ethers/viem + thirdweb Engine/OZ Defender/Biconomy).** The #1 production bug — increment-on-failure with no rollback, leaving a gap that freezes the account — afflicts alloy `CachedNonceManager` (#1073/#1126), ethers (#972), viem (#3142). Our reserve/**release**/recycle + CAS is the fix they lack and mirrors thirdweb Engine (Redis WATCH/MULTI/EXEC + recycled nonces + single-writer lock). Feature ownership:
- **Task 10 (done):** offchain counter source-of-truth · per-account single-writer atomic assign · recycle dropped nonces · reconcile-forward `reset` · durable/distributed via store swap (Phase 3) · 2D lane seam (`NonceScope.lane`, impl Phase 5).
- **Task 17 (FSM) MUST honor these contracts or the gap-freeze bug returns:** (1) call `release` on every send that will never mine (never-broadcast/dropped); (2) call `reset(chain_next)` on `nonce too low` (out-of-band tx); (3) **persist reservation before broadcast** so a crash rebroadcasts instead of leaking a gap; (4) never `release` a mined-but-reverted tx (it consumed its nonce). Plus: stuck-tx detect + same-nonce bump (Task 13/17), max-in-flight cap/backpressure, reorg resync, recover/rebroadcast.
- **Task 16/17:** idempotency — one request ≠ two nonces, keyed by `intent_hash` (persist-before-broadcast; `TxHandle.id = hash(intent_hash, nonce)`).
- **Deferred UX (note):** manual per-tx nonce override (Phase 2 `TxIntent` field); in-flight/reserved observability via the §5.6 EventSink (Task 17). Gap-filling NOOP is largely unnecessary because we **recycle** holes (no wasted gas), but remains available at Task 17 for a genuinely stuck leading nonce.

**Out-of-band txs & crash recovery — the design (research 2026-08-22, thirdweb Engine / OZ Defender / Chainstack).** Two hard cases the in-memory `{next, free}` slice does NOT itself solve; the principled (non-patch) handling:

1. **Out-of-band tx (the key signed outside the manager).** Posture: **single-writer / exclusive key ownership** is the universal production assumption — external sends are an anomaly to *detect + recover*, never a supported shared-key mode (mitigation is architectural: one signer service, not a reconciliation trick). Detection: (a) `nonce too low` at submit; (b) **periodic background reconcile** — the executor's Confirm cycle (Task 17) already reads `eth_getTransactionCount(latest)`; compare to the cached counter. **Never** re-read RPC per-allocate. Recovery: `reset(latest)` snaps the counter to chain, discards reserved nonces ≥ new count, re-drives/drops orphaned in-flight. **`latest` is authoritative; `pending` is only a forward hint** (per-node/racy). `reset` (Task 10) is the primitive; the FSM (Task 17) is the caller/cadence.

2. **Crash recovery — `{next, free}` is provably INSUFFICIENT alone.** After a crash, each reserved-but-unconfirmed nonce is ambiguous: broadcast (live tx → rebroadcast, don't reuse) vs only-reserved/dropped (dead gap → recycle). `{next, free}` can't decide it → guess wrong ⇒ frozen account or double-spend. The fix is a **third durable structure: a persist-before-broadcast WAL of the *signed* tx bytes keyed by nonce** (thirdweb `borrowed_transactions`; in walletkit = the persisted `TxHandle`, Task 17). Boot algorithm: `next = max(persisted_next, chain)` → drain WAL sorted by nonce, **rebroadcast signed bytes** (idempotent: "already known"/"nonce too low" = success) → classify accepted→in-flight vs deterministic-reject→recycle. Durable `StateStore` (Phase 3) persists `{next, free}` **and** the WAL **and** the idempotency map (key→nonce/tx_id, so a retried request never double-allocates). Persist-before-broadcast ordering is mandatory. `NonceState` itself is unchanged — the WAL is additional StateStore state added with the send pipeline (Task 16/17).
