...
## Status — IMPLEMENTED 2026-08-23 (T1–T7)

Delivered: **+20 new tests + 1 strengthened** (executor unit suite 57 → 77; localnet 8 unchanged).
Commits `cca6319` (T1) → T7. One scenario **dropped** (`recover_store_read_error_aborts_the_cycle`
— YAGNI, uninhabited `StateStoreError`); one assertion downgraded (`repeated_bumps` distinct-hash,
the mock can't escalate fees). Harness extensions added to `src/testutils.rs`: per-hash `receipts`
map, `receipt_err`, `block_numbers` FIFO. Checked boxes below are shipped.
...
- [x] **`zero_required_depth_confirms_immediately_at_the_inclusion_block`** — *medium*
...
- [x] **`replacing_reclaimed_by_our_own_mined_tx_confirms_at_depth`** — *medium*
...
- [x] **`remine_into_a_different_block_updates_tentative_mined_block`** — *medium*
...
- [x] **`receipt_read_error_yields_unknown_holds_state`** — *high* — *needs `MockRpc.receipt_err`*
...
- [x] **`bump_denied_by_fresh_policy_leaves_the_tx`** — *high*
...
- [x] **`bump_denied_when_refreshed_envelope_no_longer_admits_the_bump`** — *high*
...
- [x] **`bump_exactly_at_envelope_ceiling_is_admitted`** — *high*
...
- [x] **`newest_broadcast_receipt_wins_over_stale_older_hash`** — *medium* — *needs per-hash receipt map*
...
- [x] **`bump_then_original_mines_bump_receipt_is_ignored_original_wins`** — *medium* — *needs per-hash receipt map*
...
- [x] **`regressed_head_between_cycles_skips_confirm_and_makes_no_transition`** — *medium* — *needs sequenced head + logging nonce mgr*
...
- [x] **`receipt_missing_block_anchor_yields_unknown`** — *medium*
...
- [x] **`decode_fees_leaves_a_non_1559_signed_tx_unbumped`** — *medium*
...
- [x] **`decode_fees_leaves_a_handle_with_undecodable_signed_bytes`** — *medium*
...
- [x] **`bump_transient_submit_error_aborts_without_advancing_broadcasts`** — *medium*
...
- [x] **`repeated_bumps_across_ticks_append_broadcasts_at_the_same_nonce`** — *medium*
...
- [x] **`bump_records_broadcast_when_replacement_already_known_persists_new_hash`** — *medium* — *strengthen existing `bump_records_broadcast_when_already_known`*
...
- [x] **`recover_rebroadcasts_only_pending_and_sent_across_multiple_inflight_handles`** — *medium*
...
- [x] **`recover_swallows_a_per_handle_submit_failure_and_still_attempts_later_handles`** — *medium*
...
- [x] **`terminal_handles_are_readable_after_restart_but_not_rebroadcast`** — *medium*
...
- [x] **`reset_retains_high_freed_nonce_and_drops_consumed_freed`** — *medium*
...
- [x] **`sign_failure_recycles_nonce_leaving_no_gap_next_send_reuses_it`** — *low*
